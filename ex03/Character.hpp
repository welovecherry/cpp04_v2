...
- `Character` 객체가 생성될 때, 모든 `inventory` 슬롯은 초기에 `nullptr`로 설정됩니다.
...
특히, `Character` 클래스가 `ICharacter` 인터페이스를 구현할 때, 이 인터페이스 내의 모든 함수는 순수 가상 함수로 정의되어 있으며, 
`Character` 클래스는 이러한 인터페이스의 모든 함수를 구현해야 합니다. 여기서의 `virtual` 키워드 사용은 다음과 같은 중요한 목적이 있습니다:
...
따라서, 이러한 함수들에 virtual 키워드를 사용하는 것은, 해당 클래스가 다른 클래스에 의해 상속될 것을 고려하여 설계되었음을 나타냅니다. 
